# goals

<!-- stats:start -->
<!-- data:start -->
## attention dashboard  _(auto-updated on every commit)_
last-updated: 2026-06-16 13:10

>**areas** — last 14 days  
```
health       ░░░░░░░░░░   0 touches
career       ░░░░░░░░░░   0 touches
finances     ░░░░░░░░░░   0 touches
fun          ░░░░░░░░░░   0 touches
spiritual    ░░░░░░░░░░   0 touches
```

>**goals** — last 14 days  
```
corporal-expressiveness  ░░░░░░░░░░   0 touches
dance-bachata            ░░░░░░░░░░   0 touches
dance-cavalo-marinho     ░░░░░░░░░░   0 touches
dance-forro              ░░░░░░░░░░   0 touches
dance-frevo              ░░░░░░░░░░   0 touches
dance-salsa              ░░░░░░░░░░   0 touches
dance-samba              ░░░░░░░░░░   0 touches
exercise                 ░░░░░░░░░░   0 touches
finances                 ░░░░░░░░░░   0 touches
guitar                   ░░░░░░░░░░   0 touches
home-casinhas            ░░░░░░░░░░   0 touches
instituto                ░░░░░░░░░░   0 touches
lih-dd                   ░░░░░░░░░░   0 touches
local-ai                 ░░░░░░░░░░   0 touches
magic-places             ░░░░░░░░░░   0 touches
make-in-time             ░░░░░░░░░░   0 touches
pandeiro                 ░░░░░░░░░░   0 touches
paper-jarbinhas          ░░░░░░░░░░   0 touches
paper-megatruth          ░░░░░░░░░░   0 touches
paper-scattercode        ░░░░░░░░░░   0 touches
paper-sibgrapi-2026      ░░░░░░░░░░   0 touches
reading                  ░░░░░░░░░░   0 touches
rpg-isoroll              ░░░░░░░░░░   0 touches
sleep-regularity         ░░░░░░░░░░   0 touches
smartphone-addiction     ░░░░░░░░░░   0 touches
startapps-apptime        ░░░░░░░░░░   0 touches
startapps-arede          ░░░░░░░░░░   0 touches
startapps-batituque      ░░░░░░░░░░   0 touches
startapps-come-bem       ░░░░░░░░░░   0 touches
startapps-cuida          ░░░░░░░░░░   0 touches
startapps-da-feirinha    ░░░░░░░░░░   0 touches
startapps-gira           ░░░░░░░░░░   0 touches
startapps-laplata        ░░░░░░░░░░   0 touches
startapps-marcapramim    ░░░░░░░░░░   0 touches
startapps-permuta        ░░░░░░░░░░   0 touches
startapps-polilinhas     ░░░░░░░░░░   0 touches
startapps-realgorithm    ░░░░░░░░░░   0 touches
startapps-todolado       ░░░░░░░░░░   0 touches
startapps-ux-guidelines-ai ░░░░░░░░░░   0 touches
startapps-voti           ░░░░░░░░░░   0 touches
startapps-xacomigo       ░░░░░░░░░░   0 touches
surf                     ░░░░░░░░░░   0 touches
teaching-materials       ░░░░░░░░░░   0 touches
vipassana                ░░░░░░░░░░   0 touches
voice                    ░░░░░░░░░░   0 touches
workspace-os             ░░░░░░░░░░   0 touches
yoga                     ░░░░░░░░░░   0 touches
```
<!-- data:end -->

>**pareto**  
**sleep-regularity** — only goal with real momentum (8 touches); meta-goal that shapes energy for everything else; avoidant pattern actively being engaged  
**workspace-os** — thrilled × transformative × advancing; this system IS the infrastructure; every compass review, every ease-start lives here  
**pandeiro** — concrete Saturday anchor, lowest activation cost to show up, rhythm practice unlocks body in forró/frevo/samba simultaneously

>**gap**  
**paper-sibgrapi-2026** — URGENT: deadline May 30 (5 days), 0 touches, hard deadline no fallback — gap type: avoidant of final procedural push; message Artur now  
**paper-megatruth** — avoidant × stalled × external; team morale dropping; advisor absence compounds the problem — gap type: fear/avoidance (identity threat around research leadership)  
**career area** — 0 real touches across lih-dd, jarbinhas, megatruth — gap type: systemic drift; academic output systematically deferred
<!-- stats:end -->

---

<!-- goals:start -->
| prompt-opt-automation | career | now | auto | seed Research prompt routing / optimization state of the art | [file](goals/prompt-opt-automation.md) |

## active goals

| goal | area | horizon | timing | selected achievement | file |
|------|------|---------|--------|---------------------|------|

<!-- goals:end -->
