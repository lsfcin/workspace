...
- [ ] [workspace-os / M4-video] `core/tools/video` não lê post `/p/` de **carrossel de imagens** do Instagram — yt-dlp responde "No video formats found" por sub-item e a extração volta vazia, mesmo com cookies e login OK. Precisa de um caminho de imagem (baixar as imagens do carrossel → L3 OCR / L4 VLM). Caso de teste: `DbBDnp6DcKV`, parado no INBOX desde 2026-07-20 (2026-07-23)
- [ ] [workspace-os / M4-video] melhorar `relevance()` — hoje embeda o arquivo inteiro do goal, e goals longos/diversos (`rpg-isoroll`) viram atratores falsos pra qualquer conteúdo técnico. Ideia: descritor curto escrito à mão por goal (1-2 frases) como alvo do embedding, em vez do arquivo todo. **Confiança baixa** — um proxy dessa ideia (truncar em 250 chars ≈ título + 1º parágrafo) foi testado e também rankeou mal, então não assuma que resolve. Correção de hubness testada e rejeitada. Ver `core/tools/video.SETUP.md`. **Só vale investir se o aiwbot precisar auto-taggear captura sem agente no loop** — com agente presente, ler o texto ganha do embedding (2026-07-23)
