# [ craft | research | near ] paper jarbinhas

Jarbas finished his PhD on a design space for advanced coding tools in 3D scenes. Lots of material, nothing published yet. Lucas wants to enter the DC graduate program — needs relevant publications. Goal: be strategic, extract one or more strong papers from the PhD material.

>**signals**  
meaningful · essential · indifferent

>**timing**  
*target · within the next semester  
anchor · DC graduate program application requirements  
closure · at least one paper submitted to a relevant venue  
tolerance · moderate — no hard deadline, but delay costs program eligibility  
fallback · target lower-tier venue first to break the ice, then aim higher*

## selected next achievement
    [jarb-01] map Jarbas' PhD material — identify 2-3 publishable contributions

**ease-start**  
Read Jarbas' PhD conclusion and contributions chapter. List every claim that could stand alone as a paper contribution. That list is your starting point.

## backlog

> [ ] [jarb-01] map Jarbas' PhD material — identify 2-3 publishable contributions  
> [ ] [jarb-02] rank contributions by effort-to-publish ratio — what's closest to ready?  
> [ ] [jarb-03] pick one contribution, identify target venue  
> [ ] [jarb-04] outline paper structure from existing PhD material  
> [ ] [jarb-05] write, review with Jarbas, submit  

## done

<!-- done:start -->
<!-- done:end -->

## stats
<!-- stats:start -->
last-touch: 2026-07-14  ·  trend: advancing

| period      | touches |
|-------------|----------|
| month       |       2 |
| trimester   |       3 |
| semester    |       3 |
| year        |       3 |
| 2-year      |       3 |
| 4-year      |       3 |
<!-- stats:end -->
