...
> **linked work** — the `core/` agent-library pilot of this idea lives in an enforced per-layer frontmatter contract ([core/SCHEMA.md](../../core/SCHEMA.md)), level unified, pilot flow `compare` normalized, validation in `sync-skills` + pre-commit. Sweep tracked in [core/ROADMAP.md](../../core/ROADMAP.md). Sibling track to the `code/` module-spec gate (pre-commit §1d).
