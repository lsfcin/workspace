# [ fun | music | near ] pandeiro

Aprender o pandeiro e as percussões. Unlock rhythm — in the head, the hand, and the foot. There's already a concrete access point: Pandeirada, a pandeiro course in Várzea, classes on Saturdays. This is in the neighborhood. The only barrier is showing up.

>**signals**  
meaningful · expected · motivated

>**dynamics**  
pragmatic mode · advancing motion · intrinsic source

## selected next achievement
    [show-up] go to Pandeirada next Saturday

**ease-start**  
Look up the Pandeirada schedule and confirm the time and location for this Saturday. Then put it in the calendar. That's it — just make the appointment with yourself.

>**timing**  
*target · 3 months of regular Saturday practice  
anchor · Pandeirada classes, Várzea, Saturdays  
closure · able to hold a rhythm in forró and samba without thinking  
tolerance · missing Saturdays occasionally is fine — just keep the thread  
fallback · if Pandeirada doesn't fit, find another percussion teacher or online course*

## backlog

> [ ] [show-up] go to Pandeirada next Saturday  
> [ ] [buy-pandeiro] get a good beginner pandeiro if you don't have one  
> [ ] [month-1] complete 4 consecutive Saturday classes  
> [ ] [rhythm-feet] be able to keep foot rhythm while playing hands — the coordination  
> [ ] [play-forro] play pandeiro in a forró rhythm without stopping  
> [ ] [play-samba] play pandeiro in a samba rhythm without stopping  

## done

<!-- done:start -->
<!-- done:end -->

## stats
<!-- stats:start -->
last-touch: 2026-07-14  ·  trend: advancing

| period      | touches |
|-------------|----------|
| month       |       2 |
| trimester   |       3 |
| semester    |       3 |
| year        |       3 |
| 2-year      |       3 |
| 4-year      |       3 |
<!-- stats:end -->
