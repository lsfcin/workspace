...
|------|-------------|---------|-------------|
| [`SETUP-accounts.md`](SETUP-accounts.md) | Everything that reaches a service off this machine: web search, the shared Google OAuth behind six tools, the Forms API's separate project, the chat bot's token, and the CIn VPN. Five of the six need a human for one browser action or one password, and each says exactly which one. | web-search, google-auth, forms, bot, vpn-cin | core/tools/test/workspace/test_setup_executable.py |
...
| [`SETUP-telegram.md`](SETUP-telegram.md) | The bot that puts this workspace in a chat: a thought captured into `brain/INBOX.md` from away from the PC, and a coding agent driven from the same thread. One process, one switch, one service. | bot | core/tools/test/workspace/test_setup_executable.py |
<!-- routing:end -->
