...
|-------|-------------|---------|-------------|
| [`SETUP-accounts.md`](SETUP-accounts.md) | Everything that reaches a service off this machine: web search, the shared Google OAuth behind six tools, the Forms API's separate project, and the Telegram capture bridge. Four of the five need a human for one browser action, and each says exactly which one. | web-search, google-auth, forms, telegram-capture | core/tools/test/workspace/test_setup_executable.py |
