...
**Why it is worse than a broken mirror:** it is the only gate in the workspace that can refuse a
commit for a reason the operator cannot fix by editing anything they wrote. The stash is the
workaround, and a stash is exactly the kind of state a session loses.

**The decision is Lucas's, and it is not "make symlinks work":** a native symlink on Windows needs
Developer Mode or an elevated shell, so choosing symlinks means adding a precondition to every
install. **Copies** cost a regeneration step the mirrors already pay on every skill edit. One
sentence in `core/tools/SPECS.md`, then `mirror.sh` follows it.

## B9 — the workspace's own `verify:fast` never runs in the gate that requires it

**Symptom:** every commit here prints `⚠ make not found — verify:fast not run for workspace`. The
contract is declared (a `Makefile` with `verify-fast:`) and discovered correctly; `make` is simply
not a program this machine has.

**Why it matters:** `verify-contract` hard-blocks a `code/` project that declares no contract,
precisely so a commit cannot outrun its tests. On a machine with no `make`, every project whose
contract is a Makefile target satisfies the gate and runs nothing — the gate is fully green and
fully blind, which is the shape the OS-agnostic port exists to remove.

**Not a silent one, deliberately:** the port made the missing runner warn and name itself rather
than report `verify:fast is red`, because a red suite that never ran is the worse lie. The warning
is the honest end state; it is not the fixed one.

**Root cause:** `make` is not in `core/tools/deps.txt`, so nothing ever claimed this machine should
...
2026-08-28 · 708 tracked files scanned · **54 findings here** (2026-08-25: 33 · +21 over 3 days)
...
| Unanswered scaffold placeholders | 3 |
...
- .claude/agents/craft-high.md — 1 line(s) over the 120-column cap (first at line 7)
- .claude/agents/craft-low.md — 1 line(s) over the 120-column cap (first at line 7)
- .claude/agents/craft-medium.md — 1 line(s) over the 120-column cap (first at line 7)
- .claude/commands/craft.md — 3 line(s) over the 120-column cap (first at line 18)
- .claude/commands/drive.md — 2 line(s) over the 120-column cap (first at line 42)
- .claude/commands/research.md — 1 line(s) over the 120-column cap (first at line 53)
- .craft/commands-mirror-cost/0-clarify.md — 2 line(s) over the 120-column cap (first at line 9)
- .opencode/CONTEXT.md — 1 line(s) over the 120-column cap (first at line 96)
- .opencode/agents/craft-high.md — 1 line(s) over the 120-column cap (first at line 6)
- .opencode/agents/craft-low.md — 1 line(s) over the 120-column cap (first at line 6)
- .opencode/agents/craft-medium.md — 1 line(s) over the 120-column cap (first at line 6)
- .opencode/skills/prepare/refs/research-summary.md — 2 line(s) over the 120-column cap (first at line 6)
...
- brain/INBOX.md — 252 lines, over the 200 cap; introduced by 459e3f8 lsfcin
...
- core/hooks/SPECS.md — 325 lines, over the 200 cap; introduced by 0237832 lsfcin
...
- .zcode/skills/CONTEXT.md:2: 1 unanswered placeholder(s).
...
- .opencode/CONTEXT.md: head is 1630 tok carrying 4 constraint(s).

### Repos on an unmerged feature branch

*promote when the work is green, or say which reason applies — /roundup Phase 5*

- . — feature/os-agnostic-port is 1 ahead of main
...
2026-08-27 · `make verify-full` · **green (572 passed)**
<!-- verify:end -->
