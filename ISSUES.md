...
- The entropy block reports the close's own branch as unpromoted, every close. `roundup` regenerates
  ...
  finding saying that branch is ahead of main, and the commit carrying the block is itself the last
  thing making it true. It fired on `feature/absorb-aiwbot` on 2026-09-12 and on
  `feature/single-source-law` the same day. Two of the block's findings at any close are therefore
  bookkeeping the close created, which trains a reader to discount the number — the same defect
  `core/tools/wos/close/repomap.py` had, one step earlier in the same script.
...
2026-09-12 · 904 tracked files scanned · **1 findings here** (2026-08-31: 46 · -45 over 12 days)
...
- . — 2 merged into main: git -C . push origin --delete feature/absorb-aiwbot feature/single-source-law
...
2026-09-12 · `verify.py full` · **green (20 passed, 868 deselected in 2.10s · 868 passed in 41.20s)**
<!-- verify:end -->
