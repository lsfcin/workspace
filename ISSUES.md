...
- The entropy block reports the close's own branch as unpromoted, every close. `roundup` regenerates
  the block before it promotes, so the run that merges `feature/*` → `develop` → `main` publishes a
  finding saying that branch is ahead of main, and the commit carrying the block is the last thing
  making it true. A reader who discounts those findings discounts the whole number. Swapping the
  order does not cure it: the artifact is written on the feature branch so it rides into `develop`,
  and after promotion no commit can carry it while the Git Flow gate holds `main`. The cure is to
...
2026-09-12 · 904 tracked files scanned · **6 findings here** (2026-08-31: 46 · -40 over 12 days)
...
| Work that exists on this disk and nowhere else | 3 |
...
- . — feature/warn-universal is 1 ahead of main
...
- . — develop is 1 ahead of origin/develop
- . — feature/confident-wrongness was never pushed
- . — feature/single-source-law was never pushed
...
2026-09-12 · `verify.py full` · **green (20 passed, 868 deselected in 2.36s · 868 passed in 53.46s)**
<!-- verify:end -->
