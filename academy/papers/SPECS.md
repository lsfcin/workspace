...
Each bib entry gets `refs/<key>.yaml` (a level-2 promoted reference) — create or update it when
adding `\cite{key}`. The `post-edit` hook warns when a `.bib` is edited. **Read the ref before
making a claim about a paper.** Raw, unpromoted captures collect in `refs/REFS.md` (level-1, per the
