...
- **enigmas:** binários, 1 caixa = 2 pts (a meia caixa foi descartada: não há glifo do mesmo tamanho de ◼/◻). 1 enigma por deck de tópico, menos rnn e autopreservação, que ficam `[0] slides`. Um deck = um tópico, nunca fundir dois. Nenhum enigma foi aplicado até 23/09. A aplicação (minissistema de formulário, versões por enigma, rastreio de acesso/envio) é projeto do Lucas a partir de 30/09.
