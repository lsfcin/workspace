...
Avoid crash-prone PDF parsing in this workflow. Prefer paper metadata, abstracts, HTML pages, official docs, and web snippets. If only a PDF exists, cite the PDF URL from search metadata and mark full-text PDF parsing as blocked instead of fetching it.
...
- Keep subagent task JSON small and valid.
...
- Prefer broad guidance such as "use paper search and web search"; if a source fails, the researcher must continue from metadata, abstracts, and web sources and mark the failure as blocked.
...
After the verifier returns, verify on disk that `outputs/.drafts/<slug>-cited.md` exists.
...
When applying reviewer fixes, use small localized edits for 1-3 simple corrections. For section rewrites or more than 3 substantive fixes, read the cited draft and write a corrected full file to `outputs/.drafts/<slug>-revised.md` instead.

After applying fixes, run an explicit on-disk verification before saying the fixes landed. Use grep, diff, wc, stat, or a targeted read to prove the old unsupported wording is gone and the replacement wording exists.
