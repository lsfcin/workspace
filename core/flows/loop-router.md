...
| `research` | investigate, gather, synthesize, review literature/sources | plan→scale→gather→draft→cite→review→deliver | `core/flows/research/{deep,literature,explore,compare,recipe,replicate,review,summarize,watch,audit}.md` + `core/flows/mechanism-search.md` — pick the closest |
