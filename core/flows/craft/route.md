...
Agent delegation: use `Agent`/`subagent`/`task` when available; otherwise the user runs the named folder flow in a fresh session.
...
**Ambiguity rule:** if a task is "decide, then build," run `architecture` first (it emits an ADR), then `feature` consuming that decision. If it's "research, then build," run `research` then `feature`. Chain folders; never merge their shapes into one run.

**Guardrail — do NOT add folders casually.** A new folder is justified only when a task type needs a genuinely different *step sequence*, not merely different content. The four above are distinct shapes. Adding a fifth requires the same bar (and a `tree.md` entry).

## Step R1 — Record + hand off

- Record the choice as `subtree: <name>` in the Carry block the folder will create, and in `STATUS.md` (`subtree:` header line). For `feature`, also carry the supervision profile the feature folder's Step 0 panel fills (`io-signoff`, `arch-review`, `arch-review-supervised`).
- Dispatch: instruct the executor (or the user) to run the named folder flow with this task. The router adds nothing to the folder's own protocol — it only pins which one runs.
