...
- **Footer (µ1, Lucas's):** anchor left on the content column, source link short and centred, `lucas s. figueiredo · dc/ufrpe` right on the column edge, all 10pt grey, regular weight, lowercase, on one bottom edge; a link is named (the site, short: `medium`, `arxiv`), never a raw url, in the footer's face, size and light weight, not underlined, on the footer line, no logo, no grey wedge. On the COPY `1F3XmZmGJkBWvPMwL0cBtBN7vTKNe2KoszrzlUAP2LYE`. Open: the cover's size order (title vs subtitle, micro and macro — a discussion, not a decision).
...
## você está aqui → TASTE, round 1 answered
`taste` is the **subjective** eye; the subskills are technical. Built by interviewing him with images. Round 1 (a gallery of 38, score + why) is answered: his words, verbatim, are [`brain/drafts/taste-galeria-1.md`](../../brain/drafts/taste-galeria-1.md); the page is https://claude.ai/artifact/N1W9HSAcmBehecrXyzxZSB. Next: predict the 10 held-out items BEFORE unlocking `config/heldout` (agreement is the measure) → triads (his criteria in his words) → A/B pairs on copies of his decks → distil: a ban list, criteria as two-pole scales, his reasons; `taste.md` stays short. **Every item he scored ≥3 is a candidate slide template** (a template slide, not a template deck).
