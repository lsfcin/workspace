...
The shim exists because rtk reads the first line of a Bash payload and nothing else, which left every multi-line call uncompacted — 23.4% of them open with a `cd`. What it may split, what it hands back untouched, and the two undocumented harness facts it rests on: [`SPECS.md`](SPECS.md).
