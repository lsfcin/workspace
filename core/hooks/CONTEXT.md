...
| [`gates.txt`](gates.txt) | — | — | Every PreToolUse gate, and the capability that selects it. Read by core/hooks/dispatch.py (which runs them) and by core/hooks/trigger/trigger_law.py (which reports when they fire). |
