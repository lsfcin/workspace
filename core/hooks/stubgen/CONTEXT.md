...
| [`paper-scaffold.py`](paper-scaffold.py) | [`paper-scaffold.pyi`](paper-scaffold.pyi) | `scaffold`, `main` | paper-repo.py: Initialize or adapt a paper directory to workspace standards. new <name>    — create academy/papers/<name>/ from template adapt <path>  — add missing repo files to an existing paper Both modes are safe: existing files are never overwritten (skipped with ~). |
