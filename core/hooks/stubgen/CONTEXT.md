# stubgen
> Interface stubs and paper templates, generated on save and on commit.
...
| [`paper-template.py`](paper-template.py) | [`paper-template.pyi`](paper-template.pyi) | `apply_template`, `main` | paper-template.py: Initialize or adapt a paper directory to workspace standards. new <name>    — create academy/papers/<name>/ from template adapt <path>  — add missing template files to an existing paper Both modes are safe: existing files are never overwritten (skipped with ~). |
