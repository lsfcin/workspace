...
# half lived in a shell `case` inside read/pre-read.sh, so no Python caller could ask it and the
# CONTEXT.md gate had no way to know a stub was about to block the same Read. The reconciliation the
# comment above used to defer is this pairing: both sets here, their difference visible in one look.
