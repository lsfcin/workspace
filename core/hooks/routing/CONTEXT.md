...
| [`shard_table.py`](shard_table.py) | [`shard_table.pyi`](shard_table.pyi) | `shard_facts`, `render_table`, `shards_of`, `index_for`, `item_headlines` | A sharded type's index table: what each TYPE-<short name>.md publishes, rendered so the index answers "open or skip" without anything being opened. |
