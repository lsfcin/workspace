...
| [`context-gate.py`](context-gate.py) | [`context-gate.pyi`](context-gate.pyi) | `target_path`, `main` | PreToolUse: Read|Edit|Write|Grep|NotebookEdit — force-read the CONTEXT.md chain of the target's subtree before any other file access. Session-deduped via the marker store hook_input.store names, written by context-tracker.py. See code/ROADMAP-verify.md W1. |
| [`context-tracker.py`](context-tracker.py) | [`context-tracker.pyi`](context-tracker.pyi) | `main` | PostToolUse: Read — record CONTEXT.md/SPEC.md reads (consumed by context-gate.py / bash-context-gate.py / spec-read-gate.py) and interface-file reads (consumed by pre-read.py: interface read unlocks its source; and by pdf-gate.py: a PDF's twin read unlocks the PDF). ROADMAP-verify.md W1. |
| [`pdf-gate.py`](pdf-gate.py) | [`pdf-gate.pyi`](pdf-gate.pyi) | `reads_raw`, `pdfs_in`, `engine_installed`, `refuse`, `main` | PreToolUse: Read + Bash — a PDF is read through its twin. Fresh twin: the PDF is blocked until the twin is read, which unlocks it. Missing or stale: refused with the command that makes it, or let through when no engine is installed. |
| [`pre-read.py`](pre-read.py) | [`pre-read.pyi`](pre-read.pyi) | `codegraph_root`, `remind`, `main` | PreToolUse: Read — block a source read while its interface stub is current. Current stub: hard block (exit 2), the stub must be read first. Stale: warn and allow. |
| [`read_matrix.py`](read_matrix.py) | [`read_matrix.pyi`](read_matrix.pyi) | `column`, `enabled`, `record`, `load`, `fold` | How often each file was READ, per day, for the last year — counted at the moment of the read. |
| [`spec-read-gate.py`](spec-read-gate.py) | [`spec-read-gate.pyi`](spec-read-gate.pyi) | `find_spec_module`, `block`, `remind`, `main` | PreToolUse: Edit|Write — a spec-locked module (its CONTEXT.md carries `> spec:` and the referenced SPEC.md header is `status: locked`) requires that SPEC.md be Read this session before editing the module's files. See code/ROADMAP-spec-drive.md. |
<!-- routing:end -->
