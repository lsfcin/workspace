...
| [`pdf-gate.py`](pdf-gate.py) | [`pdf-gate.pyi`](pdf-gate.pyi) | `reads_raw`, `pdfs_in`, `refuse`, `main` | PreToolUse: Read + Bash — a PDF is read through its twin. Fresh twin: the PDF is blocked until the twin is read, which unlocks it. Missing or stale: refused with the command that makes it. |
