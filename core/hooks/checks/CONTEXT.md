...
| [`issues-gate.py`](issues-gate.py) | [`issues-gate.pyi`](issues-gate.pyi) | `fixed_ids`, `repo_root`, `has_spec`, `main` | PreToolUse: Edit|Write on ISSUES.md — flipping an issue to FIXED requires a matching regression spec (test/**/b<N>-*.*) in the same repo. See workspace ROADMAP-verify.md I2. |
