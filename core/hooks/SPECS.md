...
*Trusted 2026-09-04 and firing — measured by the post-trust probe run
([`core/experiments/zcode-hook-protocol.md`](../experiments/zcode-hook-protocol.md)), which also
confirmed the plain-text exit-2 reason reaches the agent.
...
violation authored there is caught only at commit, by git. Event remaps: no `SubagentStart` in zcode,
so `agent-context.py` rides PreToolUse `Agent|Task`; no `PreCompact`, so `precompact-wipe.py` rides
