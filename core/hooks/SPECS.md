...
| `.codex/hooks.json` + `codex/codex-policy.py` | Codex registration plus a thin `apply_patch`/Bash adapter onto canonical gates | **ENFORCED*** |

*Project hooks must be trusted in their client before they run. ZCode is measured **inert** until
trust is granted — [`core/experiments/zcode-hook-protocol.md`](../experiments/zcode-hook-protocol.md).
Codex path and payload tests run in `verify-fast`; its first client probe belongs to SETUP.
...
| Hook | Git | Claude Code | Copilot | opencode | zcode | Codex |
|------|-----|-------------|---------|----------|-------|-------|
| Pre-read (interface redirect) | — | ✅ | ✅ | ✅ | ⏸ | wired¹ |
| Pre-edit (size / description) | — | ✅ | ✅ | ✅ | ⏸ | wired¹ |
| Pre-edit facade-scan (new files) | — | ✅ | ✅ | ✅ | ⏸ | wired¹ |
| Pre-edit facade-gate (`code/` edits) | — | ✅ | ✅ | ✅ | ⏸ | wired¹ |
| Post-edit (stubs / context sync) | — | ✅ | ✅ | ✅ | ⏸ | wired¹ |
| Post-read facade-tracker | — | ✅ | ✅ | ✅ | ⏸ | — |
| Context-gate (`CONTEXT.md` chain) | — | ✅ | ✅ | ✅ | ⏸ | wired¹ |
| Bash context-gate (cat/grep bypass) | — | ✅ | ✅ (terminal hints) | ✅ (bash tool) | ⏸ | wired¹ |
| Context / interface read tracker | — | ✅ | ✅ | ✅ | ⏸ | — |
| ISSUES gate (FIXED needs a spec) | — | ✅ | ✅ | ✅ | ⏸ | wired¹ |
| Spec-read-gate (spec-locked modules) | — | ✅ | ✅ | ✅ | ⏸ | wired¹ |
| Size / facade import / stub gen / context sync | ✅ | — | — | automatic (git) | automatic (git) |
| Spec-driven new-module gate | ✅ block | — | — | automatic (git) | automatic (git) |
| Duplication gate (jscpd) | ✅ block | — | — | automatic (git) | automatic (git) |
| `verify:fast` contract gate | ✅ block | — | — | automatic (git) | automatic (git) |
| ESLint R1-R6 (TS under `code/`) | ✅ block | ✅ warn | ❌ gap | ❌ gap | ❌ gap |
| Prettier auto-format (TS under `code/`) | — | ✅ | ❌ gap | ❌ gap | ❌ gap |

¹Path/payload tests prove the Codex adapter reaches the canonical script; a trusted-client probe
earns ✅. Codex local tools expose writes as `apply_patch`, not a first-class read result, so its
post-read trackers remain unavailable. The six ❌ are the live gaps: Copilot, opencode and zcode get no lint enforcement at all, so a TS
