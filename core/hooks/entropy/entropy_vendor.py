#!/usr/bin/env python3
# Does a list assign a vendor's model where it should assign a tier? Zero-token, deterministic.
#
...
# is the assignment SHAPE the lists use for tier: `**model: …**`, the bolded directive that sits
...
def vendor_directive_hits(files: list, exempt: set) -> list:
    """List lines telling a reader which MODEL to run, instead of which tier."""
    hits = []
    for path in files:
