...
| [`entropy_list.py`](entropy_list.py) | [`entropy_list.pyi`](entropy_list.pyi) | `retired_hits`, `item_slugs`, `duplicate_slugs`, `finished_work_hits`, `unanswered_placeholders` | Tier 0 list and vocabulary checks, parsed from core/SCHEMA.md. Zero-token, deterministic. |
...
| [`entropy_vendor.py`](entropy_vendor.py) | [`entropy_vendor.pyi`](entropy_vendor.pyi) | `is_list`, `vendor_directive_hits` | Does a list assign a vendor's model where it should assign a tier? Zero-token, deterministic. |
<!-- routing:end -->
