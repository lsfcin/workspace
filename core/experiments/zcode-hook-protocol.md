...
no matcher): `core/hooks/zcode/check.sh` dumps stdin + filtered env + cwd + ppid to
`/tmp/zcode_check/`; `check-deny.sh` (plain-text stdout + exit 2) rides a sacrificial
...
- SessionStart check fired at startup: `check.sh` executed through the expanded
  `${ZCODE_PROJECT_DIR}` and wrote `/tmp/zcode_check/000_SessionStart.txt` — variable
...
- Exit-2 fidelity: the WebFetch deny check's plain-text stdout reason
  (`CHECK-DENY-PLAIN: …`) reached the agent **verbatim** as the tool error. 2A stands —
...
- Both check registrations removed from `.zcode/config.json` (2026-09-04): WebFetch is
  unblocked and `/tmp/zcode_check/` stays empty. `.zcode/SPECS.md` § Measured answers
