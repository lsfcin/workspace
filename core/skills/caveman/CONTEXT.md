...
Upstream: **[JuliusBrussee/caveman](https://github.com/JuliusBrussee/caveman)** — the caveman modes, the commit/review/compress skills, the cavecrew presets and the activate / mode-tracker / stats / statusline hooks all originate there. Vendored here and adapted; the upstream project keeps the credit for the idea and the original implementation.

A **folder-shaped** skill: `SKILL.md` is the router, everything beside it is a subfile of that router. It registers globally rather than through `.claude/skills/`, which is what puts it in every project instead of only this workspace.

Global registration, the `$HOME` symlinks and their sync command, and the local adaptations that a re-sync has to reconcile: [`SPECS.md`](SPECS.md).
