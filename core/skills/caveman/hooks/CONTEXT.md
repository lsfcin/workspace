...
These are **standalone scripts run by node/bash**, not an importable module: `~/.claude/settings.json` names four of them by absolute path (through the symlinks `core/tools/wos/sync-global-skills` creates).
That is why there is no facade here — nothing outside this directory imports it.

[`ENTRYPOINTS`](ENTRYPOINTS) declares which files get linked into `~/.claude/hooks`. Add a helper module and it stays internal; add an entrypoint and it must be listed there **and** wired in `settings.json`.

**Four entrypoints, each named by the lifecycle event that fires it** — SessionStart, UserPromptSubmit, statusLine, and the `/caveman stats` CLI. Everything else here is a helper they import: the flag API, its symlink-safe file I/O, and the three modules `stats` splits into. Which file is which is the routing table's job; its descriptions come from their own first lines.

Attribution: [`../CONTEXT.md`](../CONTEXT.md). Wiring, and the local adaptations a re-sync has to reconcile: [`../SPECS.md`](../SPECS.md).
