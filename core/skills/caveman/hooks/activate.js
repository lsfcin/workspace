...
if (INDEPENDENT_MODES.has(mode)) {
  process.stdout.write('CAVEMAN MODE ACTIVE — level: ' + mode + '. Behavior defined by the `caveman` skill subfile ' + mode + '.md (/caveman ' + mode + ').');
  process.exit(0);
}
...
// Vendored layout (this workspace): __dirname = <suite>/hooks/, SKILL.md one level up.
// Legacy plugin layout: __dirname = <plugin_root>/hooks/, SKILL.md at <plugin_root>/skills/caveman/.
// Standalone hook install: neither exists — falls back to the hardcoded rules below.
let skillContent = '';
...
for (const candidate of skillCandidates) {
  try {
    ...
    suiteDir = path.dirname(candidate);
    break;
  } catch (e) { /* try next */ }
}

// Worked examples live in modes.md so a session loads one level's examples, not six.
// Append only its "## Example" sections; the level table already came from SKILL.md.
if (suiteDir) {
  try {
    const modes = fs.readFileSync(path.join(suiteDir, 'modes.md'), 'utf8');
    const examples = modes.split(/^## /m)
      .filter(block => block.startsWith('Example'))
      .map(block => '## ' + block.trim())
      .join('\n\n');
    if (examples) skillContent += '\n\n' + examples + '\n';
  } catch (e) { /* no modes.md — SKILL.md alone is still valid */ }
}
...
if (skillContent) {
  ...
  // SessionStart injects the *behaviour*, not the invocation surface: the router's
  // argument table and the `$ARGUMENTS` placeholder only mean something when the user
  // actually types /caveman. Dropping them keeps the always-injected block lean.
  body = body.replace(/^## Routing[\s\S]*?(?=^## )/m, '');
  body = body.replace(/^Arguments: \$ARGUMENTS\s*\n+/m, '');
