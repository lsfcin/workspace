...
Split from [`../`](../CONTEXT.md) 2026-08-18, when a fourth drawing passed the crowding signal. Every module here takes values computed by [`diagram_data.py`](../diagram_data.py) or [`diagram_health.py`](../diagram_health.py) and returns HTML. **A view that reads a file has crossed the line** — it becomes a second place the workspace is measured, free to disagree with the first, which is the failure the picture exists to make visible.

Each exports `render(...)` for the drawing and `legend(...)` for the writing beneath it.
[`../diagram_page.py`](../diagram_page.py) composes them into one self-contained document and owns the shared stylesheet, so a view ships class names rather than a `<style>` block.
...
| [`diagram_routing.py`](diagram_routing.py) | [`diagram_routing.pyi`](diagram_routing.pyi) | `render`, `legend` | The routing tree: which directory routes to which, drawn from the auto-synced routing blocks. |
