...
| [`public.txt`](public.txt) | — | — | Which trees may cross into the public repo at all, and where it is checked out. Read by core/tools/wos/publish_core.py; the per-feature half of the answer lives in core/features.txt. |
| [`publish`](publish) | — | — | what crosses into the public repo, what no feature claims, and whether anything crossing carries a credential |
| [`publish_core.py`](publish_core.py) | [`publish_core.pyi`](publish_core.pyi) | `Floor`, `floor`, `tracked`, `eligible`, `Claim` | publish_core.py — what crosses into the public repo and what does not: the floor, the claims, and the import closure that turns a claim on one file into the files it cannot run without. |
