...
# global (core.hooksPath), so its ledger stage fired inside outputs/links anyway and shipped an
...
def test_the_redirect_clone_is_not_a_declared_project():
    ...
    ledger comes back and this case says so before the next push does."""
    assert 'outputs/links' not in declared_projects(WORKSPACE_ROOT)


def test_every_project_that_keeps_a_ledger_is_one_git_declares():
    """The other direction: the set is non-empty and names real project paths, so a parse rule that
    silently drifted to matching nothing would switch every nested ledger OFF unnoticed."""
...
def test_the_ledger_stage_asks_before_it_writes():
    ...
    body = source.partition('def ledger(commit):')[2].partition('\ndef ')[0]
    assert '_keeps_a_ledger(commit)' in body, 'the ledger stage writes without asking whose repo'
    assert body.index('_keeps_a_ledger') < body.index('entropy-dashboard.py'), \
...
def test_the_published_clone_carries_no_ledger():
