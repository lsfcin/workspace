...
| [`test_pointer_integrity.py`](test_pointer_integrity.py) | [`test_pointer_integrity.pyi`](test_pointer_integrity.pyi) | `check_separators`, `check_pointers` | T0 pointer-integrity check (Level 0): every relative ](path) link across CONTEXT.md / ROADMAP*.md / SCHEMA.md / AGENTS.md must resolve. Zero-token, runs in verify-fast. |
