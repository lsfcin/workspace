# T1 links: a slug stays sayable, a private thing never gets one, and a live link never moves.
import pytest
...
def test_a_slug_is_a_name_a_room_can_be_told(mapfile):
    for good in ('ai4good', 'ai4good/setup', 'ai4good/aula3', 'rva-chico', 'ppc2026'):
        links_core.validate_slug(good)


def test_a_slug_stops_at_one_level_because_nobody_recites_a_tree():
    ...
    with pytest.raises(links_core.Refused):
        links_core.validate_slug('academy/teaching/ai4good')


@pytest.mark.parametrize('bad', ['AI4Good', 'ai4 good', 'ai4_good', '-ai4good', 'ai4good/', 'olá'])
def test_a_slug_that_cannot_be_typed_from_memory_is_refused(bad):
    with pytest.raises(links_core.Refused):
        links_core.validate_slug(bad)


@pytest.mark.parametrize('slug', ['casinhas', 'casinhas-planta', 'saude-exames',
                                  'financas-irpf', 'branches/casinhas'])
def test_a_private_subtree_never_gets_a_public_slug(slug):
    """A redirect is public: a guessable slug turns 'anyone with the link' into discoverable.

    `casinhas-planta` is the case that matters and the one this check first let through — a flat
    hyphenated slug is exactly how a one-off is named, so the hyphen is a word boundary here.
    """
    with pytest.raises(links_core.Refused):
        links_core.check_private(slug)
...
def test_a_taken_slug_is_refused_rather_than_moved(mapfile):
    """A slug already said out loud to a room must not start pointing somewhere else."""
...
def test_the_map_round_trips_through_the_file(mapfile):
    ...
    assert [r['slug'] for r in rows] == ['ai4good/setup', 'rva-chico'], "sorted by slug"
...
def test_find_is_how_a_growing_map_stays_cheap(mapfile):
    links_core.add('ai4good', 'https://example.com/a', path=mapfile)
    links_core.add('rva-chico', 'https://example.com/t', path=mapfile)
    assert [r['slug'] for r in links_core.find('ai4', mapfile)] == ['ai4good']
    assert [r['slug'] for r in links_core.find('example.com/t', mapfile)] == ['rva-chico']
...
def test_check_reads_the_file_rather_than_trusting_the_writer(mapfile):
    ...
    mapfile.write_text('slug\turl\towner\tadded\n'
