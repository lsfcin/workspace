...
**Before editing any file:** read it first, grep for every caller before changing a function, and read the facade (`index.ts` / `__init__.py`) of every module you will touch.
...
**Hooks block automatically** — each explains itself and names the fix when it fires, so nothing about them is restated here. Every numeric limit lives in `core/hooks/limits.env`, and a copy of one here went five days stale before it was deleted. Reasoning: [ROADMAP-verify.md](ROADMAP-verify.md).
...
| [`freeai/`](freeai/CONTEXT.md) | Mapa das opções gratuitas para codar com IA — harnesses de CLI e IDE, provedores com camada gratuita e modelos open-weights, em tabelas comparativas com limites reais e links oficiais. |
